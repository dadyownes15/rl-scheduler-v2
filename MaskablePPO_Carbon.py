...
def train(workload, backfill, debug=False):
    ...
    for epoch in range(epochs):
        ...
        while True:
            ...
            # KEY CHANGE: Store greenRwd instead of r (same as MARL)
            ppo.remember(state, value, log_prob, ind, greenRwd, mask, device)

            if debug and t < 3 and ep_len < 5:
                print(f"    Action: {ind.item()}")
                print(f"    Green reward stored: {greenRwd:.4f}")
            ...
            if d:
                ...
                t += 1
                # Both metrics contribute to terminal bootstrap (same as MARL)
        ...
        with open('MaskablePPO_Carbon_' + workload_name + '.csv', mode='a',
                  newline='') as file:
        ...
        if (epoch + 1) % 5 == 0:
            checkpoint_path = f"{workload_name}/MaskablePPO_Carbon_checkpoints/epoch_{epoch + 1}/"
    ...
    ppo.save_using_model_name(workload_name + '/MaskablePPO_Carbon/')
