...
class Buffer():
    ...
    def store_buffer(self, state, mask1, mask2, mask3, action1, action2, action3, log_prob1, log_prob2, log_prob3, Return, advantage, job_input,
                     nums):
        self.buffer_num = self.buffer_num + nums
        self.states.extend(state)
        self.masks1.extend(mask1)
        self.masks2.extend(mask2)
        self.masks3.extend(mask3)
        self.actions1.extend(action1)
        self.actions2.extend(action2)
        self.actions3.extend(action3)
        self.log_probs1.extend(log_prob1)
        self.log_probs2.extend(log_prob2)
        self.log_probs3.extend(log_prob3)
...
class ActorNet(nn.Module):
    ...
    def getAction3(self, x, mask):
        """
        Third action head for backfill job selection.
        Uses the same observation state and similar structure to action1.
        """
        encoder_out, _, _ = self.forward(x)
        logits = self.decoder3(encoder_out)
...
class PPO():
    def __init__(self, batch_size=10, inputNum_size=[], featureNum_size=[],
                 device='cpu', debug=False, backfill=0):
        ...
        self.backfill = backfill  # Add backfill parameter
    ...
    def choose_action(self, state, mask1, mask2, mask3):
        with torch.no_grad():
        ...
        # Action 3: Backfill job selection
        with torch.no_grad():
            probs3 = self.actor_net.getAction3(state, mask3)
        dist_bin3 = Categorical(probs=probs3)
        ac3 = dist_bin3.sample()
        log_prob3 = dist_bin3.log_prob(ac3)

        value = self.critic_net(state)
        return ac1, log_prob1, ac2, log_prob2, ac3, log_prob3, value, job_input
    ...
    def act_backfill(self, state, mask3, ac3):
        probs3 = self.actor_net.getAction3(state, mask3)
        dist_bin3 = Categorical(probs=probs3)
        log_prob3 = dist_bin3.log_prob(ac3)
        entropy3 = dist_bin3.entropy()

        return log_prob3, entropy3
    ...
    def remember(self, state, value, log_prob1, log_prob2, log_prob3, action1, action2, action3, reward, mask1, mask2, mask3, device, job_input):
    ...
    def storeIntoBuffter(self, reward):
        ...
        self.buffer.store_buffer(self.states, self.masks1, self.masks2, self.masks3, self.actions1, self.actions2, self.actions3, self.log_probs1,
                                 self.log_probs2, self.log_probs3,
    ...
    def compute_actor_loss(self,
        ...
        # Only compute action3 loss when backfill == 3
        if self.backfill == 3:
            log_probs3, entropy3 = self.act_backfill(states, masks3, actions3)
            # Compute the policy loss with all three actions
            log1 = old_log_probs1 + old_log_probs2 + old_log_probs3
            log2 = log_probs1 + log_probs2 + log_probs3
            entropy = (entropy1 + entropy2 + entropy3) / 3
        else:
            # Skip action3 training for other backfill modes
    ...
    def train(self):
        ...
        masks3 = torch.cat(self.buffer.masks3, dim=0)
        ...
        actions3 = torch.cat(self.buffer.actions3, dim=0)
        log_probs3 = torch.cat(self.buffer.log_probs3, dim=0)
        ...
        for i in range(self.ppo_update_time):
            for index in BatchSampler(SubsetRandomSampler(range(len(self.buffer.states))), self.batch_size, False):
                ...
                sampled_masks3 = torch.index_select(masks3, dim=0, index=index_tensor).to(self.device)
                ...
                sampled_actions3 = torch.index_select(actions3, dim=0, index=index_tensor).to(self.device)
                sampled_log_probs3 = torch.index_select(log_probs3, dim=0, index=index_tensor).to(self.device)
                ...
                action_loss, policy_loss, entropy_loss, kl_mean = self.compute_actor_loss(sampled_states, sampled_masks1,
                                                                                sampled_masks2, sampled_masks3,
                                                                                sampled_actions1, sampled_actions2, sampled_actions3,
                                                                                sampled_advantages,
                                                                                sampled_log_probs1, sampled_log_probs2, sampled_log_probs3,
                                                                                sampled_job_inputs)
                ...
                with torch.no_grad():
                    ...
                    if self.backfill == 3:
                        log_probs3_new, _ = self.act_backfill(sampled_states, sampled_masks3, sampled_actions3)
                        log_old = sampled_log_probs1 + sampled_log_probs2 + sampled_log_probs3
                        log_new = log_probs1_new + log_probs2_new + log_probs3_new
                    else:
    ...
    def eval_action(self,o,mask1,mask2,mask3):
        with torch.no_grad():
            ...
            mask3 = np.array(mask3).reshape(1, MAX_QUEUE_SIZE)
            mask3 = torch.FloatTensor(mask3).to(self.device)
...
class PPOModelWrapper:
    ...
    def eval_action(self, o, mask1, mask2, mask3):
        ...
        """
        return self.ppo_model.eval_action(o, mask1, mask2, mask3)
...
def run_validation_episodes(ppo_model, validation_env, validation_episodes, debug=False):
    ...
    for episode in range(validation_episodes):
        ...
        while not d:
            ...
            # mask3 for backfill job selection
            if validation_env.backfill == 3:
                # For neural backfill, mask3 is created based on current queue state
                # This will be the general queue mask, refined selection happens in environment
                mask3 = lst.copy()
            else:
                mask3 = lst.copy()  # Same as mask1 for other backfill modes
            
            # Get action from policy (deterministic evaluation)
            action1, action2, action3 = ppo_model.eval_action(o, lst, mask2, mask3)
            
            # Step environment - pass action3 for backfill == 3
            o, r, d, r2, sjf_t, f1_t, running_num, greenRwd = \
                validation_env.step(action1.item(), action2.item(), action3.item())
    ...
    for epoch in range(epochs):
        ...
        while True:
            ...
            with torch.no_grad():                            # inference only
                ...
                mask3T = torch.FloatTensor(np.array(mask3).reshape(1, MAX_QUEUE_SIZE)).to(device)
                
                action1, log_prob1, action2, log_prob2, action3, log_prob3, value, job_input = \
                    ppo.choose_action(state, mask1T, mask2T, mask3T)
            
            # Store transition in PPO's rollout buffer
            ppo.remember(
                state, value, log_prob1, log_prob2, log_prob3, action1,
                action2, action3, greenRwd, mask1T, mask2T, mask3T, device, job_input
            )
            
            # Collect action stats
            trajectory_stats["actions"].append((action1.item(), action2.item(), action3.item()))
            ...
            if debug:
                ...
                epoch_action_stats["action3_choices"] = epoch_action_stats.get("action3_choices", []) + [action3.item()]
            ...
            # ----------------------------------------------------------
            # 5-c. Step the environment - pass action3 for backfill == 3
            # ----------------------------------------------------------
            o, r, d, r2, sjf_t, f1_t, running_num, greenRwd = \
                env.step(action1.item(), action2.item(), action3.item())
