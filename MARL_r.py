...
# Import the reward interface system
from reward_interface import RewardManager
...
class PPO():
    ...
    def eval_action(self,o,mask1,mask2):
        ...
        state = torch.FloatTensor(o_reshaped).to(self.device)
        mask1 = torch.FloatTensor(mask1.reshape(1,MAX_QUEUE_SIZE)).to(self.device)
        mask2 = torch.FloatTensor(mask2.reshape(1,action2_num)).to(self.device)
        action1, log_prob1, action2, log_prob2, value,job_input = self.choose_action(state,mask1,mask2)
...
def setup_experiment_directory(workload, experiment_name, description):
    """
    Create directory structure for experiment with given workload and name.
    ...
    config_snapshot_path = f"{experiment_dir}/config_snapshot.ini"
    ...
    # Update the snapshot with additional experiment metadata
    config = configparser.ConfigParser()
    config.read(config_snapshot_path)
    
    # Add a header comment with experiment info
    ...
    description_path = f"{experiment_dir}/description.txt"
...
def train(workload, backfill, debug=False, experiment_name="", description=""):
    print("Training called with reward interface")
    # ------------------------------------------------------------------
    ...
    # Configure reward system - use same config as original MARL
    reward_manager = RewardManager(config_file="configFile/config.ini")
    print(f"Using reward function: {reward_manager.reward_function.__class__.__name__}")
    ...
    print(f"  Reward function: {config.get('reward_config', 'reward_function')}")
    ...
    for epoch in range(epochs):
        ...
        while True:
            ...
            # For step-wise storage, use the step reward (greenRwd will be set correctly above)
            ppo.remember(
            ...
            # ----------------------------------------------------------
            o, r, d, r2, sjf_t, f1_t, running_num, original_greenRwd = \
                env.step(action1.item(), action2.item())
            
            # NEW APPROACH: Calculate all rewards at episode completion
            if d:  # Only calculate when episode is done
                episode_rewards = reward_manager.calculate_episode_rewards(context={'env': env})
                
                # Use the comprehensive reward calculation
                greenRwd = episode_rewards['carbon_reward']
                episode_wait_reward = episode_rewards['wait_reward'] 
                episode_total_reward = episode_rewards['total_reward']
                specialized_reward = episode_rewards['specialized_reward']
                
                # For comparison/debugging with original system
                if debug and t < 3:
                    carbon_diff = abs(greenRwd - original_greenRwd)
                    if carbon_diff > 1e-6:
                        print(f"    INFO: Carbon reward comparison - Interface: {greenRwd:.6f}, Original: {original_greenRwd:.6f}, Diff: {carbon_diff:.6f}")
                    
                    print(f"    Episode rewards: Carbon={greenRwd:.4f}, Wait={episode_wait_reward:.4f}, Total={episode_total_reward:.4f}")
                    if abs(specialized_reward) > 1e-6:
                        print(f"    Specialized reward: {specialized_reward:.4f}")
                        
                # Use episode-level rewards for tracking
                ep_ret = episode_wait_reward  # Total wait penalty for this episode
                greenRwd = greenRwd  # Carbon reward for this episode
                
            else:
                # During episode, use step rewards for immediate feedback
                greenRwd = original_greenRwd
                episode_wait_reward = r
                episode_total_reward = eta * r + greenRwd
                
            # Store the final episode reward for PPO buffer usage
            final_episode_reward = episode_total_reward if d else (eta * r + greenRwd)
                
            # Collect reward stats
            trajectory_stats["rewards"].append((r, greenRwd))
            
            # Trajectory-level bookkeeping
            ep_ret   += r if not d else 0  # Add step reward (but override with episode total at end)
            ep_len   += 1
            show_ret += r2      # additional metric for reporting
            sjf      += sjf_t   # schedule fairness proxy
            f1       += f1_t    # some QoS metric
            
            # Reward components (accumulate during episode, but use final values at completion)
            if d:
                # Use episode-level calculations
                green_reward += greenRwd
                wait_reward  += episode_wait_reward  
                epoch_reward += episode_total_reward
            else:
                # Accumulate step-wise for interim tracking
            ...
            if d:
                ...
                # Use the episode-level total reward for PPO training
                ppo.storeIntoBuffter(final_episode_reward) # final R for GAE
    ...
    legacy_model_path = f"lublin_256/MARL_carbon_v1/"
    if not os.path.exists(legacy_model_path):
        os.makedirs(legacy_model_path)
    ppo.save_using_model_name(legacy_model_path)
