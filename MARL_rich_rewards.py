...
class PerStepRewardCalculator:
    ...
    def __init__(self, config):
        ...
        self.reward_function = config.get('carbon setting', 'per_step_reward_function', fallback='co2_direct')
    ...
    def calculate_step_reward(self, job, env_context=None):
        ...
        # Clip extreme rewards to improve value function learning
        max_reward_magnitude = 10.0  # Configurable clipping threshold
...
class PPO():
    def __init__(self, batch_size=10, inputNum_size=[], featureNum_size=[],
                 device='cpu', debug=False, enable_per_step_rewards=False):
        ...
        self.enable_per_step_rewards = enable_per_step_rewards
        self.per_step_reward_calculator = None
        if enable_per_step_rewards:
        ...
        if enable_per_step_rewards:
            config = configparser.ConfigParser()
            config.read('configFile/config.ini')
            actor_lr = float(config.get('ppo', 'actor_lr', fallback=0.0001))
            critic_lr = float(config.get('ppo', 'critic_lr', fallback=0.0005))
        else:
            actor_lr = 0.0001
            critic_lr = 0.0005
    ...
    def calculate_per_step_reward(self, job, env_context=None):
        ...
        Returns:
            float: Per-step reward value
        """
        if not self.enable_per_step_rewards or not self.per_step_reward_calculator:
            # Fallback to traditional reward calculation
            return 0.0
...
def train(workload, backfill, debug=False, experiment_name="", description="", no_score=False, validate_every_n_epochs=False, enable_per_step_rewards=False):
    ...
    print(f"  Per-step rewards: {'Enabled' if enable_per_step_rewards else 'Disabled'}")
    if enable_per_step_rewards:
        reward_function = config.get('carbon setting', 'per_step_reward_function', fallback='co2_direct')
        print(f"  Per-step reward function: {reward_function}")
    ...
    ppo = PPO(
        ...
        device          = device,
        debug           = debug,
        enable_per_step_rewards = enable_per_step_rewards
    )
    ...
    for epoch in range(epochs):
        ...
        while True:
            ...
            # Calculate per-step reward if enabled and a job was scheduled
            enhanced_greenRwd = greenRwd  # Default to original greenRwd
            if enable_per_step_rewards and action1.item() < len(env.pairs) and env.pairs[action1.item()][0] is not None:
                job_for_scheduling = env.pairs[action1.item()][0]
                # Check if the job was actually scheduled by looking at its scheduled_time
                if hasattr(job_for_scheduling, 'scheduled_time') and job_for_scheduling.scheduled_time != -1:
                    ...
                    enhanced_greenRwd = ppo.calculate_per_step_reward(job_for_scheduling, env_context)
            
            # Store transition in PPO's rollout buffer (now with properly calculated enhanced_greenRwd)
            ppo.remember(
                state, value, log_prob1, log_prob2, action1,
                action2, enhanced_greenRwd, mask1T, mask2T, device, job_input
            )
                
            # Collect reward stats (use enhanced reward if available)
            trajectory_stats["rewards"].append((r, enhanced_greenRwd))
            ...
            # Reward components
            green_reward += enhanced_greenRwd   # energy-aware part (enhanced if per-step rewards enabled)
            wait_reward  += r          # waiting-time part
            epoch_reward += eta * r + enhanced_greenRwd
            ...
            if d:
                ...
                ppo.storeIntoBuffter(eta * r + enhanced_greenRwd)   # final R for GAE
        ...
        # Per-step reward statistics if enabled
        if enable_per_step_rewards:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--per-step-rewards', action='store_true', help='Enable per-step reward calculation using configurable reward functions')
    args = parser.parse_args()
    train(args.workload, args.backfill, args.debug, args.name, args.description, args.no_score, args.validate, args.per_step_rewards)